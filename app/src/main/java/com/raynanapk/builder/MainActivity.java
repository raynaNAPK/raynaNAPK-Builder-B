...
public class MainActivity extends Activity {
    ...
    private EditText appNameInput,packageInput,bannerInput,interInput;
    ...
    @Override protected void onCreate(Bundle b){
        ...
        if(hasWebContent()) showViewerV8(); else showBuilderV8();
    }
    ...
    void showViewerV8(){
        boolean needSplash=true, needAds=false;
        String bannerId="ca-app-pub-3940256099942544/6300978111";
        String interId="ca-app-pub-3940256099942544/1033173712";
        try{
            String cfg=readAsset("www/__builder_config.json");
            needSplash=cfg.contains("\"splash\":true");
            needAds=cfg.contains("\"ads\":true");
            if(cfg.contains("bannerId")){ int i=cfg.indexOf("bannerId"); int s=cfg.indexOf("\"",i+11)+1; int e=cfg.indexOf("\"",s); if(s>0&&e>s) bannerId=cfg.substring(s,e); }
            if(cfg.contains("interId")){ int i=cfg.indexOf("interId"); int s=cfg.indexOf("\"",i+10)+1; int e=cfg.indexOf("\"",s); if(s>0&&e>s) interId=cfg.substring(s,e); }
        }catch(Exception e){}
        if(needSplash){
            ...
            LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(280,280); lp.setMargins(0,0,0,30); logo.setLayoutParams(lp);
            TextView tv=new TextView(this); tv.setText(getAppName()); tv.setTextColor(Color.WHITE); tv.setTextSize(26); tv.setGravity(Gravity.CENTER);
            ProgressBar pb=new ProgressBar(this);
            TextView load=new TextView(this); load.setText("Loading Money Maker..."); load.setTextColor(Color.parseColor("#00D084")); load.setGravity(Gravity.CENTER); load.setPadding(0,20,0,0);
            splash.addView(logo); splash.addView(tv); splash.addView(pb); splash.addView(load);
            setContentView(splash);
            boolean fAds=needAds; String fb=bannerId, fi=interId;
            new Handler().postDelayed(() -> showWebV8(fAds,fb,fi), 2000);
        } else showWebV8(needAds,bannerId,interId);
    }

    void showWebV8(boolean needAds,String bannerId,String interId){
    ...
    Button makeBtn(String t){ Button b=new Button(this); b.setText(t); b.setTextColor(Color.WHITE); b.setBackgroundColor(Color.parseColor("#6C5CE7")); LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,-2); p.setMargins(0,5,0,5); b.setLayoutParams(p); b.setPadding(0,18,0,18); return b; }
}
