...
public class MainActivity extends Activity {
    ...
    private File selectedFile=null,selectedIcon=null,lastBuiltApk=null;
    private TextView logView;
    private EditText appNameInput,packageInput,bannerInput,interInput,jsInjectInput;
    private CheckBox splashCheck,fileCheck,adsCheck,aabCheck,pushCheck;
    ...
    void showViewerV9(){
        ...
        String jsInject=getCfg("jsInject","");
        String appName=getCfg("appName","MyApp");
        if(needSplash){
            ...
            TextView tv=new TextView(this); tv.setText(appName); tv.setTextColor(Color.WHITE); tv.setTextSize(28); tv.setGravity(Gravity.CENTER); tv.setPadding(0,10,0,0);
            TextView ver=new TextView(this); ver.setText("V9 EMPIRE • Play Store Ready"); ver.setTextColor(Color.parseColor("#00D084")); ver.setTextSize(12); ver.setGravity(Gravity.CENTER);
            ProgressBar pb=new ProgressBar(this);
            splash.addView(logo); splash.addView(tv); splash.addView(ver); splash.addView(pb);
            setContentView(splash);
            boolean fAds=needAds; String fb=bannerId, fi=interId; String fJs=jsInject;
            new Handler().postDelayed(() -> showWebV9(fAds,fb,fi,fJs), 2000);
    ...
    void showWebV9(boolean needAds,String bannerId,String interId,String jsInject){
        ...
        wv.setWebViewClient(new WebViewClient(){
            public void onPageFinished(WebView view, String url){
                if(jsInject!=null&&!jsInject.isEmpty()&&!jsInject.equals("null")){
                    try{ view.evaluateJavascript(jsInject,null); }catch(Exception e){}
                }
            }
        });
        ...
        if(needAds){
            try{
                ...
                FrameLayout.LayoutParams adParams=new FrameLayout.LayoutParams(-1,-2); adParams.gravity=Gravity.BOTTOM;
    ...
    void showBuilderV9(){
        ...
        TextView title=new TextView(this); title.setText("⚡ Builder B V9\n👑 EMPIRE - AAB + JS"); title.setTextSize(17); title.setTextColor(Color.parseColor("#FFD700")); title.setGravity(Gravity.CENTER); title.setPadding(0,0,0,6);
        appNameInput=new EditText(this); appNameInput.setHint("App Name"); appNameInput.setText("MyApp"); appNameInput.setTextColor(Color.WHITE); appNameInput.setHintTextColor(Color.GRAY); appNameInput.setBackgroundColor(Color.parseColor("#1A1D2E")); appNameInput.setPadding(10,10,10,10);
        packageInput=new EditText(this); packageInput.setHint("Package"); packageInput.setText("com.khozenk.myapp"); packageInput.setTextColor(Color.WHITE); packageInput.setHintTextColor(Color.GRAY); packageInput.setBackgroundColor(Color.parseColor("#1A1D2E")); packageInput.setPadding(10,10,10,10);
        bannerInput=new EditText(this); bannerInput.setHint("Banner ID"); bannerInput.setText("ca-app-pub-3940256099942544/6300978111"); bannerInput.setTextColor(Color.WHITE); bannerInput.setHintTextColor(Color.GRAY); bannerInput.setBackgroundColor(Color.parseColor("#1A1D2E")); bannerInput.setPadding(8,8,8,8); bannerInput.setTextSize(9);
        interInput=new EditText(this); interInput.setHint("Inter ID"); interInput.setText("ca-app-pub-3940256099942544/1033173712"); interInput.setTextColor(Color.WHITE); interInput.setHintTextColor(Color.GRAY); interInput.setBackgroundColor(Color.parseColor("#1A1D2E")); interInput.setPadding(8,8,8,8); interInput.setTextSize(9);
        jsInjectInput=new EditText(this); jsInjectInput.setHint("JS Inject (contoh: alert('hacked');)"); jsInjectInput.setText(""); jsInjectInput.setTextColor(Color.WHITE); jsInjectInput.setHintTextColor(Color.GRAY); jsInjectInput.setBackgroundColor(Color.parseColor("#1A1D2E")); jsInjectInput.setPadding(8,8,8,8); jsInjectInput.setTextSize(9);
        LinearLayout.LayoutParams pm=new LinearLayout.LayoutParams(-1,-2); pm.setMargins(0,4,0,4); appNameInput.setLayoutParams(pm); packageInput.setLayoutParams(pm); bannerInput.setLayoutParams(pm); interInput.setLayoutParams(pm); jsInjectInput.setLayoutParams(pm);
        ...
        adsCheck=new CheckBox(this); adsCheck.setText("💰 AdMob"); adsCheck.setChecked(true); adsCheck.setTextColor(Color.parseColor("#00D084")); adsCheck.setTextSize(11);
        aabCheck=new CheckBox(this); aabCheck.setText("📦 Build AAB (Play Store)"); aabCheck.setChecked(true); aabCheck.setTextColor(Color.parseColor("#FFD700")); aabCheck.setTextSize(11);
        ...
        Button buildBtn=makeBtn("💰 3. BUILD APK MONEY"); buildBtn.setBackgroundColor(Color.parseColor("#00D084")); buildBtn.setOnClickListener(v->{ if(selectedFile==null){ toast("Pilih file!"); return; } buildV9(false); });
        Button aabBtn=makeBtn("👑 4. BUILD AAB PLAY STORE"); aabBtn.setBackgroundColor(Color.parseColor("#FFD700")); aabBtn.setTextColor(Color.BLACK); aabBtn.setOnClickListener(v->{ if(selectedFile==null){ toast("Pilih file!"); return; } buildV9(true); });
        logView=new TextView(this); logView.setText("📝 V9 LOG:\n> AAB + JS Inject\n> V8.1 stable + new\n> Build #28 Empire!\n"); logView.setTextColor(Color.parseColor("#8B8FA8")); logView.setTextSize(9); logView.setPadding(8,8,8,8); logView.setBackgroundColor(Color.parseColor("#1A1D2E"));
    ...
    void buildV9(boolean buildAAB){
        new Thread(() -> {
            try{
                ...
                String config="{\"appName\":\""+appName+"\",\"package\":\""+packageInput.getText()+"\",\"splash\":"+splashCheck.isChecked()+",\"ads\":"+adsCheck.isChecked()+",\"bannerId\":\""+bannerInput.getText()+"\",\"interId\":\""+interInput.getText()+"\",\"jsInject\":\""+jsEsc+"\",\"version\":\"V9\"}";
                zos.putNextEntry(new ZipEntry("assets/www/__builder_config.json")); zos.write(config.getBytes()); zos.closeEntry(); zos.close();
                runOnUiThread(() -> log("> Signing V1+V2+V3..."));
                Security.addProvider(new BouncyCastleProvider()); KeyPairGenerator kpg=KeyPairGenerator.getInstance("RSA"); kpg.initialize(2048); KeyPair kp=kpg.generateKeyPair(); X500Principal issuer=new X500Principal("CN=BuilderB V9"); JcaX509v3CertificateBuilder certBuilder=new JcaX509v3CertificateBuilder(issuer, BigInteger.valueOf(System.currentTimeMillis()), new Date(System.currentTimeMillis()-100000), new Date(System.currentTimeMillis()+365L*24*3600*1000*3), issuer, kp.getPublic()); X509Certificate cert=new JcaX509CertificateConverter().setProvider("BC").getCertificate(certBuilder.build(new JcaContentSignerBuilder("SHA256withRSA").setProvider("BC").build(kp.getPrivate()))); ApkSigner.SignerConfig sc=new ApkSigner.SignerConfig.Builder("B9",kp.getPrivate(),Collections.singletonList(cert)).build(); new ApkSigner.Builder(Collections.singletonList(sc)).setInputApk(unsigned).setOutputApk(signed).setV1SigningEnabled(true).setV2SigningEnabled(true).setV3SigningEnabled(true).build().sign();
                lastBuiltApk=signed;
                runOnUiThread(() -> { log("✅ V9 "+(buildAAB?"AAB PLAY STORE":"APK MONEY")+" JADI!\n📁 "+signed.getPath()+"\n> "+(buildAAB?"Upload Play Console!":"Install & Cuan!")); toast("V9 Sukses!"); if(!buildAAB){ Intent intent=new Intent(Intent.ACTION_VIEW); intent.setDataAndType(Uri.fromFile(signed),"application/vnd.android.package-archive"); intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_GRANT_READ_URI_PERMISSION); try{ startActivity(intent); }catch(Exception e){} } });
